...
//        Short itemType = (short) 1;
    ...
    public void uploadChunk(ChunkUploadDTO chunkUploadDTO) {
        ...
        if (storedChunkCountInt.equals(chunkUploadDTO.getChunkCount())) {
            // 尝试加锁，避免重复触发合并
            String mergeLockKey = "merge:lock:" + chunkUploadDTO.getFileHash();
            boolean lockAcquired = redisUtil.tryLock(mergeLockKey, "1", 10, TimeUnit.MINUTES);
            if (lockAcquired) {
...
//            fileChunkUtil.deleteAllChunks(chunkUploadDTO.getFileHash());
                        ...
                        // 释放锁
                        redisUtil.releaseLock(mergeLockKey);
                    }
